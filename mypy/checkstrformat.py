...
def compile_format_re() -> Pattern[str]:
    ...
    format_re = (
        f'%{key_re}{flags_re}{width_re}{precision_re}{length_mod_re}{type_re}'
    )
...
def compile_new_format_re(custom_spec: bool) -> Pattern[str]:
    ...
    if not custom_spec:
        ...
        format_spec = f'(?P<format_spec>:{fill_align}{num_spec}{conv_type})?'
    else:
...
def parse_conversion_specifiers(format_str: str) -> List[ConversionSpecifier]:
    ...
    return [
        ConversionSpecifier(m, start_pos=m.start())
...
def parse_format_value(format_value: str, ctx: Context, msg: MessageBuilder,
    ...
    for target, start_pos in top_targets:
        if match := FORMAT_RE_NEW.fullmatch(target):
            conv_spec = ConversionSpecifier(match, start_pos=start_pos)
        elif custom_match := FORMAT_RE_NEW_CUSTOM.fullmatch(target):
...
class StringFormatterChecker:
    ...
    def check_specs_in_format_call(self, call: CallExpr,
        ...
        for spec, repl in zip(specs, replacements):
            ...
            if (
                ...
                and '{' not in spec.format_spec
                and '}' not in spec.format_spec
                and (
                    (
                        not custom_special_method(
                            actual_type, '__format__', check_all=True
                        )
                        or spec.conversion
                    )
                )
    ...
    def perform_special_format_checks(self, spec: ConversionSpecifier, call: CallExpr,
        ...
        if spec.conv_type == 'c':
            ...
            if (
                isinstance(c_typ, LiteralType)
                and isinstance(c_typ.value, str)
                and len(c_typ.value) != 1
            ):
                self.msg.requires_int_or_char(call, format_call=True)
        if (
            (not spec.conv_type or spec.conv_type == 's')
            and not spec.conversion
            and self.chk.options.python_version >= (3, 0)
            and (
                has_type_component(actual_type, 'builtins.bytes')
    ...
    def auto_generate_keys(self, all_specs: List[ConversionSpecifier],
        ...
        for spec in all_specs:
            if not spec.key:
                ...
                spec.field = str_index if not spec.field else str_index + spec.field
                next_index += 1
        return True
        ...
        if (isinstance(replacements, DictExpr) and
            ...
            for k, v in replacements.items:
                if (
                    ...
                    and not isinstance(k, BytesExpr)
                ):
    ...
    def check_s_special_cases(self, expr: FormatStringExpr, typ: Type, context: Context) -> bool:
        ...
        if isinstance(expr, StrExpr):
            ...
            if self.chk.options.python_version >= (3, 0) and has_type_component(
                typ, 'builtins.bytes'
            ):
                self.msg.fail(
                ...
                return False
            if self.chk.options.python_version < (3, 0) and has_type_component(
                typ, 'builtins.unicode'
            ):
                self.unicode_upcast = True
        if (
            isinstance(expr, BytesExpr)
            and self.chk.options.python_version >= (3, 0)
            and has_type_component(typ, 'builtins.str')
        ):
